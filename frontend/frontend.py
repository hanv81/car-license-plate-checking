import cv2, requests, datetime, os, traceback
...
def main():
    ...
    if access_token:
        ...
        with tab2:
            show_history(username, headers)

        with tab3:
            show_config(headers)

        with tab4:
            show_statistic(headers)

def show_plates(headers):
    cols = st.columns(2)
    with cols[0]:
    ...
    if response.status_code != HTTPStatus.OK:
        ...
        with st.form('Create'):
            ...
            if st.form_submit_button('Create', use_container_width=True):
                response = requests.post(API_URL + 'create_user', headers = headers, params={'username': user, 'password': pw})
                if response.status_code == HTTPStatus.OK:
